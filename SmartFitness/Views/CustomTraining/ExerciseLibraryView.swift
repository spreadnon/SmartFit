...
private struct ExerciseLibraryRow: Identifiable {
    let id: String
...
struct ExerciseLibraryView: View {
    ...
    var body: some View {
        ZStack {
            ...
            confirmButtonSection
                ...
                .allowsHitTesting(!selectedExercises.isEmpty)
    ...
    private var exerciseListView: some View {
        LazyVStack(alignment: .leading, spacing: 32) {
            ForEach(sortedGroupKeys, id: \.self) { key in
                exerciseGroupView(key: key)
            }
        }
    }

    @ViewBuilder
    private func exerciseGroupView(key: String) -> some View {
    ...
    @ViewBuilder
    private func exerciseRowsView(for key: String) -> some View {
        VStack(spacing: 16) {
            ForEach(exerciseRows(for: key)) { row in
    ...
    private func exerciseRows(for key: String) -> [ExerciseLibraryRow] {
        let exercises = groupedExercises[key] ?? []
        return stride(from: 0, to: exercises.count, by: 2).map { index in
            let rowExercises = Array(exercises[index ..< min(index + 2, exercises.count)])
    ...
    private func isExerciseSelected(_ exercise: LibraryExercise) -> Bool {
